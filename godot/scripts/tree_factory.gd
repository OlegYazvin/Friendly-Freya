...
static func _ensure_materials() -> void:
	...
	_shadow_mat.albedo_color = Color(0.05, 0.1, 0.05, 0.22)
	_shadow_mat.transparency = StandardMaterial3D.TRANSPARENCY_ALPHA
	_shadow_mat.shading_mode = StandardMaterial3D.SHADING_MODE_UNSHADED
	_shadow_mat.cull_mode = StandardMaterial3D.CULL_DISABLED
...
static func create_tree(pos: Vector3, tree_scale: float, rng: RandomNumberGenerator) -> Node3D:
	...
	for i in range(3):
		...
		branch.rotation_degrees = Vector3(-35.0 + rng.randf_range(-8.0, 8.0), float(i) * 120.0 + rng.randf_range(-14.0, 14.0), 0.0)
